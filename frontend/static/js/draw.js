...
function _drawPDZones(W, H) {
  ...
  for (let k = 0; k < rngs.length; k++) {
    ...
    if (x1 <= x0) continue;
    const last = (k === rngs.length - 1), w = x1 - x0;
    const fa = last ? 0.07 : 0.03;   // 溢價/折價填色：歷史更淡
    drawCtx.fillStyle = `rgba(239,83,80,${fa})`; drawCtx.fillRect(x0, yTop, w, yEq - yTop);   // 溢價(上半紅)
    drawCtx.fillStyle = `rgba(38,166,154,${fa})`; drawCtx.fillRect(x0, yEq, w, yBot - yEq);    // 折價(下半綠)
    drawCtx.setLineDash([5, 4]); drawCtx.lineWidth = last ? 1 : 0.6;
    drawCtx.strokeStyle = `rgba(255,214,79,${last ? 0.7 : 0.3})`;                             // EQ 50%
