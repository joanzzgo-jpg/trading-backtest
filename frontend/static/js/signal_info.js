...
(function () {
  ...
  const SIGNAL_INFO = {
    ...
    ss1: {
      name: "SS1（SS 系列・深）",
      subtitle: "布林軌道反轉 2 棒｜收盤靠下/上軌（較深）",
      icon: "⇋",
      color: "#05d9e8",
      gist: "軌道反轉 2 棒：下跌K(綠)插軌、上漲K(紅)收回軌內 → 反轉到中軌。<b>SS1＝B 棒收盤落在「軌道↔中軌」靠軌道的那一半（較深的反轉）。</b>SS 為獨立系列，自己一套合計與「敗後停手」，不與 S1~S12 合併、不計入 S 總勝率。（紅K=漲、綠K=跌）",
      patterns: [
        { dir: "做多（下軌反轉，訊號棒=B 紅K）", cond: "A 棒=綠K（跌），B 棒=紅K（漲）且收盤 > 下軌；A 或 B 任一根 low 觸下軌；<b>收盤 &lt; (下軌+中軌)/2</b>" },
        { dir: "做空（上軌反轉，訊號棒=B 綠K）", cond: "A 棒=紅K（漲），B 棒=綠K（跌）且收盤 < 上軌；A 或 B 任一根 high 觸上軌；<b>收盤 &gt; (上軌+中軌)/2</b>" },
      ],
      ...
      notes: [
        ...
        "與 SS2 同一觸發、依收盤深度區分：SS1=靠軌道（深）、SS2=靠中軌（淺）",
      ],
    },
    ss2: {
      name: "SS2（SS 系列・淺）",
      subtitle: "布林軌道反轉 2 棒｜收盤靠中軌（較淺）",
      icon: "⇌",
      color: "#39ff14",
      gist: "與 SS1 完全相同的軌道反轉 2 棒觸發，<b>差別只在 B 棒收盤深度：SS2＝收盤落在「軌道↔中軌」靠中軌的那一半（較淺的反轉，回中軌空間較小）。</b>SS 為獨立系列，獨立合計與敗後停手，不計入 S 總勝率。（紅K=漲、綠K=跌）",
      patterns: [
        { dir: "做多（下軌反轉，訊號棒=B 紅K）", cond: "A 棒=綠K（跌），B 棒=紅K（漲）且收盤 > 下軌；A 或 B 任一根 low 觸下軌；<b>(下軌+中軌)/2 ≤ 收盤 &lt; 中軌</b>" },
        { dir: "做空（上軌反轉，訊號棒=B 綠K）", cond: "A 棒=紅K（漲），B 棒=綠K（跌）且收盤 < 上軌；A 或 B 任一根 high 觸上軌；<b>中軌 &lt; 收盤 ≤ (上軌+中軌)/2</b>" },
      ],
      excludes: ["訊號棒 B 已碰中軌（多：high ≥ 中軌；空：low ≤ 中軌）→ 無空間，跳過"],
      entry: "訊號棒 B 下一根開盤（i+2）",
      stop:  "A、B 兩棒最低（多）／最高（空） × (1 ± SL buffer)",
  ...
  const _S_KEY_MAP = { abc: "abc", ab: "ab", s3: "3", s4: "4", s5: "5", s6: "6", s7: "7", s8: "8", s9: "9", s10: "10", s11: "11", s12: "12", ss1: "ss1", ss2: "ss2" };
