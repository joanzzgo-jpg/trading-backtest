...
function _mainChartPrice(el) {
  ...
  // 收斂後的現價（單一權威來源，見 _pushCurPrice）；沒有就退回主圖最後一根
  if (_curPx.key === key && _curPx.price != null) return _curPx.price;
  /* ★★ 2026-09-25 使用者：「按標的 a 切回標的 b，b 的合約行情價格會跳回上一秒之前的再回來」。
     根因不在這裡的年齡閘門，而在 **`/api/ohlcv` 有 30 秒 TTL 快取**（limit 查詢）：
     切走再切回同一檔就吃到快取，最後那根**形成中**的 K 棒是當初抓的 ——
     實測 ARK 1m 等 24 秒再要一次，ohlcv 末根 0.2389、同一刻 /api/latest 是 0.2379，**差 0.42%**。
     而下面那個年齡閘門看的是**棒的時間**（1m 就是整分鐘、日線是今天 00:00），
     它**量不到取樣新舊**，所以 30 秒前的價照樣過關 → 被當成現價寫進那一列
     （`_paintTickerRow` 還會 `t.price = _mp` 寫回資料）→ 畫面上就是往回跳一下再回來。
     → 改成「這批 ohlcv 是不是剛抓的」才准用它當現價（後端回的 `ts`，快取命中時回當初那一刻）。
     ⚠ 1.5 秒＝兩支端點各自 1 秒節奏，差一拍是常態，不必為此放棄主圖同步（同上方註解的寬容值）。
     ⚠ 不新鮮就回 null → 那一列用**自己的**即時報價（1 秒一次，永遠不會往回走）；
       主圖那邊 1 秒內就會有 /api/latest 推進 `_curPx`，之後照樣由主圖領軍，不留空窗。 */
  const _ots = +window._ohlcvTs || 0;
  if (!_ots || Date.now() / 1000 - _ots > 1.5) return null;
