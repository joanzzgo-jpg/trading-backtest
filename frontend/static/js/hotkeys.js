...
(function () {
  ...
  const RESERVED = {
    "0": "選快捷繪圖工具", "1": "選快捷繪圖工具", "2": "選快捷繪圖工具", "3": "選快捷繪圖工具",
    "4": "選快捷繪圖工具", "5": "選快捷繪圖工具", "6": "選快捷繪圖工具", "7": "選快捷繪圖工具",
    "8": "選快捷繪圖工具", "9": "選快捷繪圖工具",
    "[": "上一個時間框架", "]": "下一個時間框架",
    // ⚠ M 是 ui.js 的磁吸切換（同樣走 _physKey 的全域單鍵）。不列進來的話，使用者可以把
    //   某個動作綁到 M → 按一下**同時**切磁吸又觸發那個動作，而且兩邊都「正常運作」、零錯誤。
    //   ★ 新增任何全域單鍵時，這張表要一起加，否則就會開放出一顆會雙重觸發的鍵。
    "m": "磁吸（吸附到 K 棒的開高低收）",
  };
  ...
  document.addEventListener("keydown", (e) => {
    ...
    /* 1~9 / 0 ＝快捷繪圖列的第 1~10 顆工具（2026-09-22 使用者：「鍵盤1,2...-0改成快捷繪圖
       用的按鈕」，並說原本的切時框「用不太到」）。時框仍可用 [ ] 與左右方向鍵切。
       ⚠ 走 window._qdPick（ui.js）→ 它內部用 btn.click() 重用既有的 [data-tool] 事件委派，
         行為跟滑鼠點一模一樣。**別在這裡另外寫一份「設定工具」的邏輯**：兩份一定會分家。
       ⚠ 沒選到（那一格沒工具／draw.js 還沒載入）就**不要** preventDefault —— 讓鍵回到瀏覽器，
         否則會變成「按了沒反應、也沒人處理」。 */
    if (K >= "0" && K <= "9") {
      const picked = (typeof window._qdPick === "function") ? window._qdPick(K === "0" ? 10 : +K) : null;
