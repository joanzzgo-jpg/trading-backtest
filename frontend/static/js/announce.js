...
(function () {
  const PUB_ID   = "2026-09-08-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-09-08";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-09-08", "👥", "看得到別人在同一個標的上畫了什麼",
     "新功能。左側繪圖工具列 →「共享繪圖」，兩個**分開的**開關：\n\n・**看別人的**：純讀，對別人沒有任何影響。他們的線畫在你的下層、半透明，一眼看得出不是你畫的，左上角標作者。\n・**分享我的**：預設**關**。而且是**逐標的**的——勾選框只管你現在看的這一檔，換一檔就是另一個答案。關掉立刻收回。\n\n你既有的繪圖不會因為這個功能被動上傳，沒有任何一條路徑會自動分享。別人的線也**碰不到**——選不到、拖不到、刪不到，也不會混進你的存檔。\n\n⚠ 本站的身分只有帳號名稱、沒有密碼，分享出去的內容請當成公開資訊。"],

    ["2026-09-08", "🇹🇼", "台股少了 745 檔——ETF、興櫃、還有沒成交的股票",
     "你回報「有些沒中文名」，追下去發現不是名稱的問題，是那些標的**整批不在清單裡**。三個各自獨立的原因：\n\n・**5~6 碼的 ETF**：程式只收 4 碼代號 → 00878 國泰永續高股息、006208 富邦台50、00919、00631L 正2… 全部不存在（351 檔）。\n・**當天沒成交的股票**：清單只來自「每日收盤行情」，沒成交就整檔消失，例如 6949 沛爾生醫（34 檔）。現在改成只要有掛牌就在，價格顯示「---」。\n・**興櫃**：整批 349 檔都沒有，例如 7887 宇川精材。現在收進來了，代號後面標「興」。\n\n清單 1955 → **2700 檔**。興櫃排在上市櫃之後——它**沒有漲跌幅限制**，混在一起排會佔滿漲幅榜前段（實測前 25 名被它拿走 6 席），把真正的漲停股擠下去。"],

    ["2026-09-08", "🕐", "交易時段的直立線拿掉了，改成浮水印",
     "原本每個盤開盤處有一條**貫穿整張圖的直立線**，把畫面切得零碎。\n\n現在改成在該盤色塊的**最高點上方**寫「LDN 2PM」這樣的浮水印——資訊沒有變少，但圖乾淨多了。盤名改用交易圈通用的英文簡寫（ASIA／LDN／NY，股市是 TW／EU／US）。"],

    ["2026-09-08", "📱", "手機上不用滑就看得到收盤價和漲跌幅",
     "手機的開高低收那一列放不下、要橫向捲，而**漲跌幅排在最後面**——一眼最想看的數字反而要滑到底才看得到。\n\n把「收盤價 + 漲跌幅」排到最前面了。實測 390px 螢幕：以前不捲只看得到「開高低收」，現在是「收 79,603.9 / −12.2(−0.02%) / 開 / 高」。"],
  ];
