...
(function () {
  const PUB_ID   = "2026-07-14-2";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-07-14";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-07-14", "🪟", "新功能：多圖分割佈局（桌面）", "頂部工具列新增「分割」鈕：單圖 → 主圖＋1 迷你圖 → 主圖＋3 迷你圖 循環。迷你圖即時跳動（現價＋漲跌%、每 5 秒更新 K 棒）、可各自切時框（15m/1H/4H/1D）；點迷你圖的標的名稱＝與主圖「交換」——主圖維持全功能（策略標記／繪圖／勝率），迷你圖走輕量模式不搶效能。佈局與清單會記住。"],
