...
(function () {
  const PUB_ID   = "2026-09-24-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ...
    ["2026-09-23", "⏱", "狀態列多了「這根 K 棒還有多久收」",
     "上方狀態列（連線訊號與「已儲存到雲端」之間）新增**最新 K 棒倒數**，依你目前的時間框架算 —— 看 5 分線就是 5 分鐘的倒數，看 1 分線就是 1 分鐘。\n\n・收盤那一刻**不會閃一下空白**：歸零後直接跳回一整根的長度。\n・休市或資料中斷時就空著不顯示 —— 不會端一個假的倒數給你。\n・那塊位置的寬度是固定的，秒數在跳也不會把右邊的「已儲存到雲端」推來推去。", "新功能"],

    ["2026-09-23", "📶", "連線訊號和閃爍綠點合併成一個符號",
     "原本是分開的兩塊，現在併成一個「圓點＋漸高格子」的符號，但**兩種功能各自獨立**：\n・**綠點**（最左邊）＝資料有沒有在進來，收到新報價就閃一下。\n・**四格**＝連線品質，滑鼠移上去會顯示「連線 4/4 格（回應 7ms）— 順暢」。\n\n綠點縮到跟格子一樣寬、貼齊同一條基線，讀起來是連貫的一條斜坡。", "調整"],

    ["2026-09-23", "🕐", "1 分線的十字線時間看不出幾點幾分",
     "在 1 分線上把游標移到某一根，下方時間軸的標籤只寫「09-22」—— 明明是分鐘圖，卻看不出是幾點幾分。現在會寫成「**09-22 23:06**」。\n\n原因是決定「要不要顯示時分」的那份時框清單**漏掉了 1 分線**（還留著早就移除的時框）。現在改成「只要不是日線以上就顯示時分」，往後新增任何盤中時框都自動帶時分。\n\n順帶修掉：電腦睡醒後有時會誤報「圖表已中斷」—— 時間換算差了整整 8 小時，讓它以為 K 棒老了 8 小時。", "修好了"],
  ];
