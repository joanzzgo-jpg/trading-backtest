...
(function () {
  const PUB_ID   = "2026-07-18-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-07-18";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-07-18", "📊", "新指標：訂單簿 DOM（主圖右側盤口階梯）", "時框行新增「📊 訂單簿」按鈕（預設關、僅加密貨幣）：主圖右側浮出即時盤口階梯——各價位「還掛著、等待成交」的買/賣掛單，綠條=買方掛單（往左延伸）、紅條=賣方掛單（往右延伸），橫條長度＝掛量，現價置中、上方賣單下方買單。橫條最長的那一列，就是大額資金當下的防守（買方）／壓力（賣方）位置。表頭顯示現價、價階與掛單買賣比，價幅會自動貼合盤口實際深度。與「🧱 掛單牆」共用同一份盤口資料（兩者同開不會多耗），搭配一起看更能抓到「市價單吃穿掛單」的位置。⚠ 提醒：這顯示的是『當下還掛著』的單、不保證會成交（可能被撤）；要分辨真防守或假單，請配合掛單牆的「✓ 吃掉／⚠ 撤走」判定。"],
