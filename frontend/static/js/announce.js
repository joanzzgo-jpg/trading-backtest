...
(function () {
  const PUB_ID   = "2026-07-08-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ...
    ["2026-07-08", "📐", "繪圖不再跑掉", "修正切換時間框後繪圖線偏離原價位、切大時框線消失、以及線變無限長橫跨畫面等問題；繪圖現在精準跟隨價格軸縮放"],
    ["2026-07-08", "🧹", "主圖精簡＋更快", "移除已無實測優勢的舊 CRT 訊號與 KDJ/共振/ATR/回測標記，主圖只留 FVG 與 SS 軌道反轉；勝率計算與深歷史抓取一併加速、切回舊標的更順"],
