...
(function () {
  const PUB_ID   = "2026-07-31-3";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ...
    ["2026-07-31", "🇹🇼", "台股 4 小時線不再落後", "台股的 4 小時線以前盤中會停在舊值——它是用另一個比較慢的來源組出來的，而且系統把它當成「不用即時更新」的時框，所以載入之後那根就一直不動。現在改用跟 5 分／15 分線同一個即時來源，並且會持續更新。下一個交易日盤中，4 小時線的收盤價應該要跟 15 分線對得起來，不會再差一截。"],
    ["2026-07-31", "👥", "多人同時使用時不會互相拖累", "報價每秒更新一次，以前每多一個人在看就多一份對交易所的請求，人一多就會撞到交易所的流量上限（一旦被擋，全站行情會停 60 秒）。現在同一秒內的請求會合併成一次，實測八個人同時看的情況下，對交易所的請求從 8 次降到 1 次，單次回應也從 111 毫秒降到 15 毫秒。你自己看到的即時性完全沒變。"],
    ["2026-07-31", "🩹", "副圖平移更順", "打開下方 KDJ／RSI／MACD 之後左右平移，以前每次重新排版都會連帶重建一組「只用來對齊時間軸、不含任何價格」的隱藏資料——時間軸根本沒變，等於白做。現在時間軸沒變就跳過，實測每次重排從 15.7 毫秒降到 8.3 毫秒，K 棒越多省越多。"],
  ];
