...
(function () {
  const PUB_ID   = "2026-07-11-10";    // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ["2026-07-11", "🌀", "颱風資訊", "有颱風接近時，天氣資訊卡會顯示：颱風名稱／編號、在你哪個方向多遠、往哪移動、台灣有無陸上颱風警報、近中心風速（資料來自日本氣象廳 JMA 全球颱風＋中央氣象署 CWA 台灣警報）。台灣發布陸上颱風警報（或中心極近）時，背景也會自動轉成暴風。"],
