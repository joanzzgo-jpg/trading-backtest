...
(function () {
  const PUB_ID   = "2026-09-16-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-09-16";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-09-16", "🔃", "上下顛倒：手機也有按鈕了，電腦按 A 就切",
     "上次加的「上下顛倒」（整張圖倒過來、上漲顯示成下跌，用來檢查自己的多空偏見），原本手機只能從「設定」分頁進去——看盤看到一半要跳兩個分頁再跳回來，很不順。\n\n・**手機**：圖例列（BB／VOL／FVG 那排）最右邊多了 **⇅** 按鈕，而且固定貼在右邊，橫向捲動時也不會跑掉。\n・**電腦**：直接按 **A** 切換，再按一次恢復（原本的 Alt/Option + I 也還在）。中文輸入法下按同一顆鍵一樣有效，不用切輸入法。\n\n開著的時候按鈕會亮橘色，圖上方也一直掛著「上下顛倒中·點此恢復」——不會讓你忘記自己正在看反的。"],

    ["2026-09-16", "📉", "換一個沒看過的標的，少傳三成資料",
     "點開一個新標的時，訊號那包資料裡其實有一大塊是**你畫面上根本看不到的圖層**：預設隱藏的進出場標記、只剩主控台能開的研究用標記、已經從圖例移除的舊標記，還有一份前端完全沒在用的資料。\n\n實測佔整包的 **29%**。現在改成「沒在顯示的圖層就不跟伺服器要」，打開任一層時才補抓那一層。\n\n同一個請求實測 **少 26～39%**（手機用行動網路時最有感）。勝率數字、K 棒、FVG、多空標記逐一比對過，跟以前完全相同。"],

    ["2026-09-16", "🚀", "第一次打開快了一秒",
     "全新裝置（或清過快取）第一次打開，從 4.2 秒縮到 **3.15 秒**。三件事：\n\n・**城堡圖被下載了兩次**——網址被自動轉義成兩種寫法，變成同一張 81KB 的圖抓兩遍，而且就卡在最關鍵的那段。畫面上完全看不出來。\n・**裝飾用的程式讓路**：天氣、繪圖那些（約 132KB）原本跟 K 棒資料同時在搶頻寬，現在等 K 棒出現後才載。\n・**首屏再瘦 68KB**：掛單、我的交易、多圖、農民曆等「點了才用得到」的面板移出首屏，改成閒置後才載。\n\n（每天開 App 的情況上次已經改成 0.4 秒秒開，這次是針對第一次。）"],
  ];
