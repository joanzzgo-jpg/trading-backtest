...
(function () {
  const PUB_ID   = "2026-07-07-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-07-07";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ...
    ["✨", "三策略匯流高亮", "當『多空・順多空・破多空』三種同方向策略落在連續兩根 K 內 → 那兩根 K 棒霓虹發亮（空紅／多綠），一眼抓三重共振點"],
