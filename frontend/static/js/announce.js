...
(function () {
  const PUB_ID   = "2026-09-22-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-09-22";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-09-22", "📐", "盈虧比大改：整個盒子終於搬得動了",
     "・**搬得動**：以前在色塊裡按住拖曳，其實是在拉其中一條線 —— 想平移整個盒子卻把比例拉爛（實測 1:1 被拖成 **1:0.31**），而且畫面上沒有任何錯誤、只是你的盈虧比壞了。現在離線遠一點按下去就是整盒平移，比例不變。\n・**畫法改了**：第二點固定是**停損**（先定風險），停利自動放在 2 倍風險處，之後再拖調整。停損點在進場上方就自動變成做空的盒子。\n・**固定上下、滑動進場**：拖中間那條進場線時，上下的停利停損**釘住不動**，盈虧比跟著變 —— 壓力位、支撐位都已經畫在圖上了，剩下的問題就是「進場放哪裡划算」。\n・盒子上多一行「報酬 +x.x%　風險 −x.x%」，而且縮放 K 棒時那行字不會再跑掉。", "調整"],

    ["2026-09-22", "✏️", "拖曳畫好的線時，十字線不再凍住",
     "按住任何一個繪圖在調整的時候，十字線和右側的價格標籤整段都是停住的 —— 正在調價格，卻看不到游標對應的價是多少。\n\n原因是拖曳時把滑鼠事件擋掉了（本意是不讓圖表跟著被拖走），結果連圖表自己的十字線也一起斷了。現在十字線照常跟著游標，圖表也一樣不會被拖動。", "修好了"],

    ["2026-09-22", "📊", "斐波那契：右邊一大片「看不見卻點得到」",
     "・**幽靈點擊區**：斐波的線只畫到右端點，但它的點擊判定**右邊完全沒有邊界** —— 實測線畫在畫面左側，右邊三分之二的任何地方按下去都會選到／拖到它，而那裡明明什麼都沒有。\n・**小幣的價格標籤整排是 0**：小數位是照價格級距猜的，0.00001234 直接顯示成「0.0000」。現在跟著資料實際用到的小數位走。\n・另外，畫第二點的過程中就看得到各層級了（以前要放手才出現）。", "修好了"],

    ["2026-09-22", "⌨️", "數字鍵 1~0 改成選繪圖工具",
     "數字鍵原本是切時間框架。現在改成選**上方快捷繪圖列**由左到右第 1~10 顆（0＝第 10 顆），按鈕右下角有對應的小數字。\n\n時間框架仍然可以用 `[` `]` 或左右方向鍵切換，沒有失去任何操作。\n\n順手修掉：拖曳快捷繪圖列的時候會冒出說明文字、擋在你要放的位置上 —— 現在按著滑鼠就不會跳提示。", "調整"],

    ["2026-09-22", "🔃", "「上下顛倒中」換了樣子",
     "以前是圖頂正中一顆實心橘色膠囊、整顆就是按鈕，顏色寫死，跟 app 其他地方不同調。\n\n現在改成**整張主圖描一圈琥珀外框**，左上角一個小標籤「⇅ 上下顛倒中」加一顆「恢復」鈕：提示的範圍更大、更不可能忘記，但完全不遮到 K 棒。顏色也跟著你的系統配色走。\n\n（倒過來的圖若被當成正常的看，漲跌會整個讀反 —— 所以這個提示是刻意醒目的，不會做成低調的樣子。）", "調整"],

    ["2026-09-22", "🇹🇼", "台股收盤後，203 檔興櫃停在前一個交易日",
     "台股清單的底稿永遠落後一個交易日，今天的價是另外疊上去的 —— 收盤後要靠「補齊」把整份清單補成當天的收盤價。\n\n補齊的進度原本記的是**「我打算問的那批」**而不是「對方真的回答的那批」：整批抓取失敗時（上游連不上）照樣被記成「補過了」，一輪就湊滿 2698 檔、當天不再補。日誌上寫的是「收盤補齊完成：2698 檔／1 輪」，看起來完全正常。\n\n現在來源沒有真的回答，進度就不前進，下一輪重問、網路一通就自己補上。實測 **203 檔 → 2 檔**（剩下兩檔是台指期夜盤還在跳）。", "修好了"],
  ];
