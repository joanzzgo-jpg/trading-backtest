...
(function () {
  const PUB_ID   = "2026-09-18-2";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ["2026-09-18", "📣", "更新資訊改成隨時打得開，還能看快捷鍵與歷史",
     "以前公告只會自動跳一次、關掉就沒了。現在它是一個有三個分頁的面板：\n・**最新消息**：這幾天的更新（就是你正在看的這頁）。\n・**快捷鍵**：全部 13 組快捷鍵一次列出來（切時框、重播、繪圖圖層、上下顛倒…）。\n・**更新紀錄**：以前的公告，預設列近 7 天、只顯示標題，點標題才展開細節，下面可以再載更早的。\n\n打開方式：電腦按右上「系統外觀」裡的「更新資訊 · 快捷鍵」，手機在「設定」分頁；鍵盤直接按 **?** 會開到快捷鍵那頁，按 **Esc** 關掉。", "新功能"],

    ["2026-09-18", "🎨", "公告卡片改成跟 app 同一套風格",
     "原本那張卡是封面城堡的羊皮紙佈告（米紙底、紙紋、手繪虛線框、歪斜的草寫日期），放在 app 裡看起來像另一個程式的東西。\n\n現在改成跟系統外觀、交易面板同一套：深色面板、圓角、同款邊框與陰影，**顏色直接吃你自己的系統配色** —— 你換配色或切到極簡（白）模式，這張卡會跟著一起變。", "調整"],

    ["2026-09-18", "🛠", "極簡模式下繪圖、交易、通知、公告全都沒反應",
     "極簡（最省電）模式原本只打算跳過特效與天氣動畫，但那段程式的位置剛好擋住了**所有**延遲載入的檔案 —— 後來陸續搬過去的繪圖工具、交易面板、通知、更新公告、農民曆…在極簡模式下等於一個都沒載入。\n\n症狀是「按鈕都在、按了完全沒反應」，而且不會跳任何錯誤，所以一直沒被發現。現在極簡模式只會少掉特效與天氣這兩支，其餘功能都正常。", "修好了"],
