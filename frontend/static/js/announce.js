...
(function () {
  const PUB_ID   = "2026-09-21-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-09-21";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-09-21", "⌨️", "快捷鍵可以自己改了",
     "右上「系統外觀」多了一顆 **⌨️ 快捷鍵 · 可自訂**（鍵盤直接按 **?** 也會開到那一頁）。\n\n可以改的有 8 組：搜尋、重播、上下顛倒、復原、繪圖圖層 A／B／C，還有這張說明表自己。點要改的那一列，再按你想用的鍵就綁好了，每列右邊可以單獨還原，下面有「全部還原」。\n・**綁到已經有用途的鍵會擋下來**，並告訴你那顆鍵現在是做什麼的（數字鍵＝切時框、`[` `]`＝上下一個時框、**M**＝磁吸）。\n・存的是**鍵盤上的位置**、不是打出來的字 —— 中文輸入法下綁的鍵，切回英數一樣有效。\n・會跟著帳號同步到別台裝置，拉下來**當場生效**，不用重新整理。", "新功能"],

    ["2026-09-21", "✏️", "上方那排繪圖快捷，自己挑要放哪幾支",
     "開高低收右邊那排繪圖工具，最後面多了一顆 **⋯**：點開就能勾選這排要放哪些工具、順序也可以自己排，用不到的取消勾選就收起來。\n\n預設仍是原本那 7 支（游標、趨勢線、水平線、矩形、圓形、路徑、文字），按「還原預設」隨時回去。這份設定同樣會跟著帳號同步，別台裝置拉下來立刻重畫。", "新功能"],

    ["2026-09-21", "🇹🇼", "台股報價：興櫃接上了，收盤後也不再停在昨天的價",
     "・**興櫃 343 檔終於有價**：興櫃在報價來源要用不一樣的代號前綴，我們一直拿上市櫃的格式去問 → 整批查無，那 343 檔顯示的始終是落後一個交易日的價。\n・**收盤後顯示的是昨天的收盤與昨天的漲跌**：台股清單的底稿本來就永遠落後一個交易日，今天的價是另外疊上去的 —— 而底稿每 30 秒整包覆蓋一次，就把疊上去的洗掉了。現在會記住疊過哪些、每次覆蓋後重貼。\n・**凌晨部署重啟後 1191 檔退回前一天**：收盤補齊原本綁在「現在幾點」的時間窗，窗外重啟就補不到。改成看「最近一個交易日補齊了沒」。\n・報價主源換成一次能問 500 檔的批次介面，全台股覆蓋一輪從約 40 秒縮到 **15 秒**；但不假設它一定即時 —— 盤中會量它自己帶的時間戳，落後超過 3 分鐘就自動換回備援。\n\n上線後跟官方收盤價逐檔比對過。", "修好了"],

    ["2026-09-21", "📐", "可見範圍的最高最低，多了「次高／次低」",
     "主圖原本會標出「你現在看得到這一段」的最高與最低價，現在多標**第二高與第二低**（線比較細，跟主要的高低分得出來）。\n\n次高／次低會跟最高／最低保持距離（至少 3 根 K 棒，或這段可見根數的 5%），不會兩條線黏在一起、變成在講同一個位置。", "新功能"],

    ["2026-09-21", "🔍", "右邊那行價格不再疊在一起看不清",
     "你回報「畫水平線之後，右邊價格那行會重疊」。四個來源一次修掉：\n・**水平線的價格標籤互相疊**：現在會先把靠在一起的標籤分群、平均排開再畫，密集的地方也讀得出來。\n・**價格軸的刻度被蓋在標籤底下**：現價與十字線的價格標籤是我們自己畫的，圖表庫不知道它們存在、照樣在同一個位置畫刻度 —— 兩個數字疊在一起（橘色的現價上緣透出灰色的刻度）。現在被蓋住的那一格不畫。\n・**十字線價格壓到現價**：游標移到現價附近時，先把現價那顆收起來，移開再出現。\n・**字太小**：右側價格文字 10 → **11.5px**；現價標籤的底色從 30% 提高到 **70%**，不再透出底下的 K 棒。", "修好了"],

    ["2026-09-21", "📶", "左上角多了連線狀態；電腦睡醒 K 棒沒補回來會告訴你",
     "・**連線四格**移到「已儲存到雲端」左邊，閃爍的綠點移到它右邊。格數的門檻是**實際量這個服務的往返時間**訂出來的（滿格 <120ms／3 格 <250ms／2 格 <600ms／1 格 更慢或心跳停了／0 格 離線）—— 原本的門檻讓幾乎所有人恆為滿格，等於沒有資訊。\n・**不知道的時候會說不知道**：還沒量到往返時間、或超過 20 秒沒有新樣本，四格會變成空心，不會假裝健康。\n・**電腦睡著又醒來**：即時更新一中斷，中間那幾根 K 棒就再也補不回來，而畫面上完全看不出來。現在會自己偵測（看計時器有沒有被凍結過），醒來後先給 12 秒讓它自動補，**真的沒補回來**才在圖上方出現一條提示，附「重新整理」按鈕；補回來的話自己收掉。收盤後或週末醒來不會誤報。", "新功能"],

    ["2026-09-21", "💬", "滑鼠停在按鈕上，說明字 0.8 秒就出現",
     "你回報「鼠標移到物件上，太久才顯示小字提示」。那個延遲是瀏覽器寫死的（大約 1～2 秒）、改不了，所以自己畫一個：**0.8 秒**就出現，樣式也跟 app 一致。\n\n移開、按下滑鼠、捲動或切到別的視窗都會立刻收起來；觸控裝置維持原本的行為（長按叫系統選單）。", "調整"],

    ["2026-09-21", "🚀", "程式與樣式檔再小 20%",
     "改成開機時就先壓好（brotli 最高等級），請求進來直接送那一份，不再每次現場壓：\n・主程式 100.3 → **79.6KB**\n・樣式表 31.7 → **23.8KB**\n\n除了少傳 20.3% 的位元組，也省掉每個請求現場壓縮的時間。", "更快了"],
  ];
