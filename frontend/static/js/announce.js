...
(function () {
  const PUB_ID   = "2026-09-26-2";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-09-26";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-09-26", "🍎", "整個介面對齊了一套設計系統",
     "以前**每個元件各自為政**：光是毛玻璃就有 13 種模糊強度、動畫曲線有 7 條只用過一兩次的、字距不分大小一律收緊。現在收斂成一組固定刻度。\n\n・**毛玻璃五級**（極薄→極厚），浮動面板一律吃同一套材質，上緣多了一道玻璃高光邊。\n・**字距跟著字級走**：小字加空氣、大字收緊、超過 20px 又放鬆一點 —— 這是蘋果系統字看起來「對」的核心機制，以前完全沒有。\n・**同心圓角**：內圓角 = 外圓角 − 內距。工具島裡的按鈕、分段切換的選中片，角度終於跟外框對得上。\n・彈出面板的圓角、陰影、材質統一（系統外觀與提示鈴以前是另一種圓角）。", "調整"],

    ["2026-09-26", "📱", "手機：按鈕更好點，而且點完不會卡在「被選取」的樣子",
     "・**點完會卡住**：以前在手機上輕點上方那排圖示鈕，它會停在放大＋橘色光暈的狀態，要點別處才恢復 —— 看起來像那顆被選起來了。那是桌機的滑鼠效果跑到觸控裝置上，現在只在有滑鼠的裝置才出現。\n・**按鈕放大到 44×44**（蘋果的觸控下限）：上方那排圖示鈕與時框鈕都達標；星號、圖表類型這幾顆則是**外觀不變、感應範圍變大**。實測不合格的從 20 個降到 8 個。\n・按下去的回饋統一了（以前有 9 種不同的縮放幅度）。", "修好了"],

    ["2026-09-26", "♿", "系統的「降低透明度 / 提高對比」現在有反應了",
     "如果你在手機或電腦的輔助使用裡開了這兩個開關，以前這個 app **完全沒有回應**。\n\n・**降低透明度** → 所有毛玻璃變成不透明實色，天氣不再從面板後方透出來，小字更好讀。\n・**提高對比** → 次要文字與邊框加深（實測對比從 5.3:1 提到 8.3:1，達到最嚴格的 AAA 等級）。\n・在系統設定裡**中途打開**也會立刻跟著變，不用重整。", "新功能"],

    ["2026-09-26", "🌤", "天氣：國外的夜晚被畫成白天、濕度永遠 0%",
     "在**台灣、香港、日本以外**的地方（那三地走各自的氣象局，不受影響）：\n\n・**夜晚被當成白天**：實測紐約當地 23:27、倫敦 04:27，程式都判成白天 —— 晴朗的夜晚會畫出白天的天空。\n・**濕度永遠 0%**：根本沒去跟資料源要那個欄位（修好後是 44% / 61% / 62% 這種正常值）。\n・剛好 0°C 會顯示成 20°C；濃霧（能見度 0）會顯示成 10 公里。\n・**颱風方位遠距離會算錯**：從紐約看西太平洋的颱風算成「正東方」，其實路徑是往西北經過北極上空。\n・高緯度地區（像冰島夏天日落在午夜之後）太陽會卡在地平線不動。", "修好了"],

    ["2026-09-26", "💹", "合約行情列表更好讀了",
     "・**分隔線改成內縮**、對齊標的名的左緣，不再把頭像一起切開（這是蘋果清單最好認的細節）。\n・**選中的那一列**改成內縮的圓角色塊，不再是「整列滿版底色＋左邊一條直條」兩種樣子疊在一起。\n・**標的名被截斷的情況少了一半**（實測 40 列中 30 列 → 15 列）：頭像與間距都收緊，空間讓給名字。\n・上方的「合約／台股」與排序切換改成同心圓角，選中的那一片終於跟外框貼合。", "調整"],
