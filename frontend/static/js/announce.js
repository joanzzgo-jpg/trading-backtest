...
(function () {
  const PUB_ID   = "2026-07-16-1";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-07-16";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ["2026-07-16", "📡", "雷達偵測：雨雲在你正上方", "「即將降雨預警」再加一層雷達：有時雨不是從別區移過來，而是**直接在你頭頂上生成**——周圍雨量站全乾、雨卻馬上落下。現在接上全球雷達合成回波，你座標正上方（約 1 公里精度）已有回波時，天氣卡與小啊會立刻警示「雷達顯示雨雲已在你正上方（大雨強度），雨可能隨時落下」——比雨量計快一步（雨量計要等雨落地累積才知道）。上線當天實測：台北雨量站還沒反應、雷達已測到頭頂 45dBZ 大雨，預警正確發出。"],
    ...
    ["2026-07-15", "🔋", "低電量／弱效能裝置平移更順", "天氣背景的「自適應降幀」擴到桌面版：拖曳、縮放主圖時背景動畫自動讓路（分三級節流、停手即恢復滿幀），筆電開低電量模式或效能較弱的裝置平移明顯更順。報價列與行情更新不受影響、照常每秒跳動。"],
