...
(function () {
  const PUB_ID   = "2026-09-24-2";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ["2026-09-24", "💾", "開過的圖表，歷史資料會留在你的裝置上",
     "以前每次打開同一個標的，都要把整段歷史 K 棒重新下載一次 —— 但那些是**永遠不會再變的資料**。\n\n・現在第一次看過之後就存在你的裝置裡，**第二次打開同一個標的／時間框架少下載 70~80% 的資料**（實測 235KB → 45~71KB）。\n・只存「已經定案」的區間 —— 最近幾小時還在變動的 K 棒不會被存起來，不會出現「數字卡在舊的、要清快取才好」。\n・手機開「資料節省模式」或在 2G/3G 下，這些背景預載會**自動停掉**。", "更快了"],

    ["2026-09-24", "⭐", "自選的標的會先幫你把歷史載好",
     "背景預載以前只認三檔寫死的幣（BTC/ETH/XAUT），**台股、美股、港股完全不在名單上**。現在改成看你自己的習慣：\n\n・**自選裡的標的優先**（最多前 8 檔）—— 點開就已經有圖。\n・再來是你**真的花時間在看**的標的（用停留時間算，不是用點開次數 —— 翻找標的時連點十幾檔不該被當成常用）。\n・**時間框架也會學**：只有你真的切過去看的那幾個才會連訊號一起預載，只看日線的人不必為了用不到的時框付流量。\n・背景請求的頻率完全沒有變快，只是排隊順序更聰明。", "新功能"],

    ["2026-09-24", "🪶", "台股清單與圖片都瘦身了",
     "・**台股報價清單**：2700 檔裡每一列的代號都被送了兩次（完全相同的兩個欄位）—— 拿掉之後整包**省 13.6%**，而它是全站最大的一份資料。\n・**所有圖片**重新無損壓縮，合計**少 38%**（143.6KB → 88.6KB）。是無損的：每一張都逐位元比對過，畫面完全一樣。", "更快了"],

    ["2026-09-24", "🎨", "配色改好了、隔天開又變回紅色",
     "改成黑色的主圖背景，下次打開又是紅的 —— 根因是**手機/平板和電腦各自存了一份色盤**。iPad 一律算「手機款」，所以永遠讀到手機那份（紅色），電腦那份（黑色）它根本看不到。\n\n・兩份色盤已經**合併成一份**，手機和電腦現在共用同一組配色。\n・還有一個更隱蔽的：電腦上那顆早就沒人在用的舊色盤，每次同步都會被原封不動推回雲端，把你在 iPad 上剛改好的顏色蓋回去 —— 現在會自動清掉。\n・在手機上改完設定**馬上切出 app**，那次變更以前會送不出去（iOS 會把切到背景時還沒送完的請求直接砍掉）→ 改用瀏覽器保證送達的方式，現在切出去也存得住。", "修好了"],

    ["2026-09-24", "🔔", "提示鈴改版：一次看到所有標的的鬧鐘",
     "以前鈴鐺只列「目前這一檔」的鬧鐘 —— 換個標的就看不到，重新整理後更像整個消失了。\n\n・現在會列出**所有標的**的鬧鐘，每一筆都標著是哪一檔、哪個價位。\n・點清單裡任一筆，直接跳到那個標的。\n・鈴鐺上的數字＝目前還在等的鬧鐘總數。", "新功能"],

    ["2026-09-24", "⚡", "到價通知變快，換標的不再打網路",
     "到價通知的網路請求從**每次 5 個減到 1 個（−80%）**；而且切換標的時**完全不打網路** —— 清單早就整份拿回來了，換一檔只是在你的裝置上篩一下。", "更快了"],

    ["2026-09-24", "☁️", "設定也會從本機同步到線上了",
     "以前只有繪圖會從本機同步上去，設定（配色、線寬、快捷鍵…）不會。\n\n・現在**設定也會**，而且是**鏡像**：你在本機刪掉的設定，線上那份也跟著刪 —— 不會留著一個你以為早就清掉的舊值。\n・繪圖仍然是**合併**不是鏡像：本機畫過的那幾檔以本機為準，本機沒碰過的標的保持原樣（那可能是另一台裝置畫的）。", "新功能"],

    ["2026-09-24", "⏱", "K 棒倒數放大、移到時間框右邊",
     "倒數從時間框**裡面**移到整個時間框的**右邊**，字也放大了，最後 10 秒會轉成強調色比較好抓。\n\n・順手修掉它在 1536 寬（常見的 1920×125% 筆電）把上方那一列擠成兩行的問題。", "調整"],
