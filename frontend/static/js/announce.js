...
//   ★ 寫法（2026-09-26 使用者：「公告的撰寫需簡潔有力好懂」）：
//     標題一句話講**使用者看得到的結果**，不要寫我們做了什麼；內文最多 2~4 條，
//     每條一行講完；技術原因只在「能幫助理解」時留一句，數字挑最有感的一個就好。
//     ⚠ 別把 commit 訊息搬過來 —— 那是寫給開發者的。
...
(function () {
  const PUB_ID   = "2026-09-26-3";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ["2026-09-26", "🎆", "點空白處：晚上放煙火，白天噴煙霧",
     "・晚上點一下會炸開一朵煙火，有尾跡、末段還會閃。\n・白天改成一股噴出來的白煙 —— 亮的天空下煙火看不出來。\n・高解析螢幕上不再糊（以前只畫了一半的像素）。", "調整"],

    ["2026-09-26", "📊", "主圖上那排數值不再互相推擠",
     "VOL 從「4.581」跳成「117.5K」時，後面每一個項目都會跟著左右移動。\n\n・現在每個數字各自佔好位置，最寬的情況也不動（位移 5~6px → **0**）。", "修好了"],

    ["2026-09-26", "⏱", "倒數歸零後，新 K 棒換得更快",
     "倒數到 0:00 的那幾秒改成每 0.3 秒追問一次，新棒一到就停。\n\n・倒數從 4:55 起跳是**誠實的**：新棒要等它真的到貨（比收盤晚 3~5 秒），顯示 5:00 反而是騙人的。", "更快了"],

    ["2026-09-26", "🍎", "整個介面統一成一套刻度",
     "毛玻璃、圓角、字距、動畫曲線以前**每個元件各寫各的**（光毛玻璃就 13 種濃度）。\n\n・字距會跟著字級變：小字鬆一點、大字收緊 —— 這是蘋果系統字好看的關鍵。\n・圓角有了「內外同心」：按鈕在框裡終於貼合。", "調整"],

    ["2026-09-26", "📱", "手機按鈕更好點了",
     "・點完**不會再卡在「被選取」的樣子** —— 那是滑鼠效果跑到觸控裝置上。\n・按鈕放大到 44×44（蘋果的下限），不合格的從 20 個降到 8 個。", "修好了"],

    ["2026-09-26", "♿", "支援系統的輔助使用開關",
     "在手機或電腦的輔助使用裡開「降低透明度」「提高對比」「減少動態」，畫面會跟著變 —— 以前完全沒反應。\n\n・降低透明度 → 毛玻璃變實色，小字更好讀。\n・提高對比 → 文字與邊框加深（5.3:1 → 8.3:1）。\n・減少動態 → 天氣停成一張靜止的畫面，轉場全部關掉。", "新功能"],

    ["2026-09-26", "🌤", "天氣修正（只影響國外）",
     "台灣、香港、日本走各自的氣象局，不受影響。\n\n・國外的**夜晚以前會畫成白天**。\n・濕度永遠顯示 0%。\n・颱風方位在很遠的時候會算錯。", "修好了"],

    ["2026-09-26", "💹", "合約行情更好讀",
     "・分隔線內縮，不再把頭像一起切開。\n・選中的那一列改成圓角色塊。\n・**標的名被截斷的少了一半**（40 列中 30 → 15）。", "調整"],
