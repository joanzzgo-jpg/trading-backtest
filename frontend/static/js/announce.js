...
(function () {
  const PUB_ID   = "2026-08-01-2";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  const PUB_DATE = "2026-08-01";       // 卡片右上顯示的日期
  ...
  const UPDATES = [
    ...
    ["2026-08-01", "📊", "修好：台股切到 30 分 / 2 小時，看到的其實是日線",
     "這是這批最嚴重的一個，而且完全不會報錯。台股切到 30 分或 2 小時線時，圖上畫的其實是**日線資料**——只是標籤寫著 30m/2h。原因是程式裡列「哪些時框要抓分鐘資料」的清單漏了這兩個，它們就掉進日線那條路，再被一個「認不得就當日線處理」的轉換函式原封不動放行。因為圖上仍有 K 棒、也沒有任何錯誤訊息，很難發現。修好後以台積電為例，最後一個交易日：30 分線 10 根、2 小時線 3 根（先前都只有 1 根，跟日線一模一樣）。8 小時線同樣受影響，一併修好。"],
    ["2026-08-01", "🇹🇼", "修好：台股 K 棒不穩定（這次是真的修到圖上了）",
     "你先前回報過「台股 K 棒會往回退、重整又跑回來」，當時的修正其實只套用到勝率計算那條路徑，**畫在圖上的那份資料沒有吃到**——所以你看到的圖還是舊行為。原因是當日即時與歷史來自兩個來源，舊做法是「以即時來源最早那根切一刀、整段蓋掉」，只要即時來源瞬間抓不到，當日就整段退回比較落後的那一個。現在兩條路徑用同一套合併規則：抓不到就沿用今天最後一次成功的那份，並逐欄位合併。實測 5 分線在即時來源失敗時，最後一根仍停在 13:30 收盤，不會退回 13:20。"],
    ["2026-08-01", "🕐", "修好：台股日線收盤後會少一整個交易日",
     "日線資料來源在台股收盤後要很久才補上當天那根——實測週五 13:30 收盤後，直到隔天凌晨 3 點都還只到前一天。那天台積電收 2425（漲停），日線圖上卻停在 2205，而且週線、月線因為由日線組成，同樣少一截。現在會用證交所/櫃買中心的官方資料補上最新那個交易日。補的成本是零：這兩份官方資料本來就在背景抓來維護報價清單，順手存下來即可。"],
    ["2026-08-01", "📈", "修好：上櫃股成交量一直是 0，盤中還顯示昨天的收盤價",
     "全部台股清單 1972 檔裡有 877 檔成交量顯示 0，全部是上櫃股——程式讀錯了欄位名稱。影響不只是「依成交量排序時上櫃股永遠墊底」：系統只會對「成交量最大的前 120 檔」疊上即時價，上櫃股量永遠是 0 就永遠輪不到，所以**盤中看上櫃股，價格是昨天的收盤價**。修好後量為 0 的剩 0 檔，像中美晶從 0 變成 9263 萬股，依量排序進得了前段。"],
    ["2026-08-01", "⚡", "自動交易下單更快，台股報價也更即時",
     "程式每次跟交易所或資料來源要資料時都重新建立一次加密連線，光「握手」就佔掉大半時間。改成連線建立一次就重複使用之後：下單/查詢從 155 毫秒降到 45 毫秒、台股即時報價從 85 毫秒降到 13 毫秒。對自動交易來說這段延遲就是滑價——行情在動的時候，晚 100 毫秒送出的市價單成交價會不一樣。另外台股清單的來源檔案其實一天只更新一次，卻每 30 秒重抓一次（每天約 1.2GB），現在會先問「有沒有變」，沒變就跳過。"],
  ];
