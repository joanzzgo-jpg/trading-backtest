...
(function () {
  const PUB_ID   = "2026-09-24-3";     // ⚠ 只有「發公告」時才 bump（換任意新字串即可）→ 觸發全裝置重跳
  ...
  const UPDATES = [
    ...
    ["2026-09-24", "📐", "主圖跟副圖的格線終於對得上，而且會貫穿",
     "主圖與下方 KDJ/RSI/MACD 的**背景格線一直對不齊**，看起來像四張各自獨立的圖。\n\n・原因是每張圖的右側價格軸寬度不一樣（因為刻度數字長短不同）→ 同一個時間點落在不同的位置，最多差 **11.6 像素**。現在四張圖的價格軸統一寬度，時間點完全對齊。\n・面板之間的縫隙縮到只剩那條分界線（1px），上下的格線因此成為**同一條直線**；「中間仍有區隔」也還在。\n・**圖表區的底色變得完全均勻**：以前把主圖背景調透明時，後方天氣天空的漸層會透上來，造成上半部比下半部亮、副圖又是另一個色。現在圖表區後面是一片平的純色，不論透明度調到多少，主圖與三個副圖都同一個顏色。\n・代價：主圖後方不再看得到 3D 天氣場景（天氣仍在上方列、標的列、行情列呈現）。", "修好了"],
