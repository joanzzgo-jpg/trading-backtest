...
function _applyChartBgGradient(color) {
  ...
  /* ⚠ 2026-09-24：這裡曾經依不透明度內插加一層 `backdrop-filter` 壓暗，
     是為了「主背景透明度調到 0 變白」那個回報 —— 但那個白**根本不是這裡造成的**
     （真凶是 ui.js 把帶 alpha 的值設給 document.body，露出瀏覽器預設白畫布，已修）。
     那層濾鏡唯一的效果就是**讓主圖後方的天氣更暗**，已整個移除。
     ★ 教訓：為了一個還沒確認根因的症狀加的「補償」，在找到真凶之後要記得拆掉，
       否則它會以另一個症狀的形式留在系統裡。 */
  const _bd = "";
  ...
  if (cc) {
    ...
    if (!ul) {
      ...
      ul.id = "chartUnderlay";
      /* ⚠ 2026-09-24 這裡試過改成 z-index:1（蓋住 weatherStage）來解決
         「主圖調透明度時上下顏色不一」——確實完全均勻了（四種不透明度下上/中/下像素完全相同），
         但代價是**主圖後方的天空物件全部消失**（使用者立刻回報）。已還原。
         ★ 結論：**主圖區的天氣場景比「顏色絕對均勻」重要**。上下色差來自天空本身的垂直漸層
           （上亮下暗），那是「看得到後面」的必然結果 —— 100% 不透明時就沒有色差，
           想要均勻就把不透明度調高，不要拿掉天氣。別再試著把底墊提上來。 */
