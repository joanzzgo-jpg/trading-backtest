...
_COACH_DEFAULT = {"on": False, "dirs": "both", "entry": "limit",
...
def _clean_fvg(p: dict) -> dict:
    ...
    o["universe"] = "top20" if p.get("universe") in ("top20", "top60") else "watchlist"   # 舊top60→top20相容
    return o
...
def _clean_coach(p: dict) -> dict:
    ...
    o["on"] = bool(p.get("on"))
    o["entry"] = p.get("entry") if p.get("entry") in ("market", "limit") else "limit"
    if p.get("dirs") in ("both", "long", "short"):
        o["dirs"] = p["dirs"]
    o["usdt"] = _num(p.get("usdt"), 50.0, 1.0, 100000.0)
    o["lev"] = _num(p.get("lev"), 3, 1, 50, True)
...
def _exec_signal_for_account(name, cfg, market, exchange, symbol, tf, k, d, sig, all_signals=None):
    ...
    try:
        if k == "fvg":
            ...
            # 教練限價版由 place_coach_limit 在進場區掛限價 → 市價路徑不下單（同 FVG limit 作法）。
            if cfg.get("entry") == "limit":
                return
            # 教練：已在掃描階段過濾(default時框/stage≥5/現價在區內) → 此處無 sigs/tfs 閘門。
        ...
        if symbol not in owner_syms:                 # universe=top20(FVG) → 比對成交量前20;否則自選
...
_universe_cache = {"ts": 0.0, "syms": []}   # 成交量前N(預設20)加密永續宇宙，24h 快取
...
_FVG_STABLE_SYMS = {"BTCUSDT","ETHUSDT","BNBUSDT","XRPUSDT","ADAUSDT","LTCUSDT","BCHUSDT","TRXUSDT"}
...
def place_fvg_limit_ladder(name, cfg, market, exchange, symbol, tf, gap):
    ...
    try:
        ...
        if symbol not in {(w.get("symbol") or "") for w in fvg_account_symbols(name, cfg)}:
            return                                   # universe=top20 → 比對成交量前20;否則自選
        bsym, scale = client.resolve_symbol(symbol)
        ...
        try:
            ...
            _dir_n = _c.execute(                                                          # 同向倉數（cap8 用）
                f"SELECT COUNT(*) FROM trade_log WHERE source='auto' AND sig='fvg' "
                f"AND status IN ('pending','open') AND acct={_ph} AND dir={_ph}", (name, d)).fetchone()
        finally:
            _c.close()
        if _dup:
            return
        if _open_n and _open_n[0] >= int(cfg.get("maxPos", 15) or 15):
            return
        if _dir_n and _dir_n[0] >= 8:      # 同向持倉上限 cap8：防崩盤全多/全空齊爆
            return
        W = top - bot
        wr = W / (top if want == "long" else bot)
        # 寬度上限：穩定大幣(BTC/ETH/BNB…) 2%；高波動幣 3%（回測驗證：高波動幣 2-3% 缺口仍有 edge）
        _wr_cap = 0.02 if symbol in _FVG_STABLE_SYMS else 0.03
        if wr > _wr_cap:
            return
        _mid = (top + bot) / 2.0
        ...
        if _fvg_gap_already_settled(symbol, gap.get("t"), stop, tp, want):
            ...
            return
        risk_usd = cfg.get("riskUsd") or 0
        # sweep 加碼：sweep 缺口 & 寬度 1-2% 甜蜜點 → 風險 × sweepBoost（預設 1.0=關，開啟前先小額測）
        _boost = float(cfg.get("sweepBoost", 1.0) or 1.0)
        if _boost != 1.0 and gap.get("sweep") and 0.01 <= wr < 0.02:
            risk_usd = risk_usd * _boost if risk_usd > 0 else risk_usd
        lev_cap = max(1, min(int(cfg["lev"]), 50))
        mid = _mid
...
def reconcile_coach_pending(market, exchange, symbol, tf):
    """教練限價版生命週期對帳（逐帳號）：
    ① pending 且已成交(有持倉) → 掛 SL/TP closePosition + status→open。SL 掛不上→自癒重試→仍失敗市價平倉。
    ② 過期(48h 未成交) → 撤單 expired。③ 幽靈殘單(交易所無此單、無持倉、未過期、>180s) → 清除以利重掛。絕不拋例外。"""
    try:
        if market != "crypto":
            return
        for name, cfg in get_all_auto_cfgs():
            try:
                ...
                conn, ph = _acct._db()
                try:
                    rows = conn.execute(f"SELECT id, bsym, dir, sl, tp, extra, ts FROM trade_log WHERE source='auto' AND sig='coach' AND status='pending' AND acct={ph} AND symbol={ph}", (name, symbol)).fetchall()
                finally:
                    conn.close()
                if not rows:
                    continue
                ...
                for row_id, bsym, d, sl_s, tp_s, extra_s, row_ts in rows:
                    ...
                    try:
                        ...
                        ex = {}
                    oid = str(ex.get("oid")) if ex.get("oid") else None
                    try:
                        resting = {str(o["orderId"]) for o in client.open_orders(bsym)}
                        _pos = next((p for p in client.positions() if p["symbol"] == bsym and (not _hedge or p.get("posSide") == _psd)), None)
                        has_pos = _pos is not None
                    except bt.TradeError:
                        continue
                    expired = (time.time() - (row_ts or 0) > 48 * 3600)
                    if has_pos:
                        close_side = "SELL" if want == "long" else "BUY"
                        sl_px = client.quantize_price(bsym, float(sl_s) * scale)
                        tp_px = client.quantize_price(bsym, float(tp_s) * scale)
                        ...
                        try:
                            ...
                            try:
                                if not _hedge: client.cancel_all_algo(bsym)
                                sl_oid = client.place_close_trigger(bsym, close_side, sl_px, "sl", position_side=_psd).get("orderId")
                            except bt.TradeError:
                                sl_oid = None
                        if not sl_oid:
                            ...
                            if (not past_stop) and mark > 0 and nfail < 5:
                                ...
                                c3, ph3 = _acct._db()
                                try:
                                    c3.execute(f"UPDATE trade_log SET extra={ph3} WHERE id={ph3}", (json.dumps(ex), row_id)); c3.commit()
                                finally:
                                    c3.close()
                                print(f"  ⟳ 教練止損暫掛不上(現價未到止損)、保留重試 {nfail}/5：{bsym}")
                                continue
                            try: client.close_position(bsym, position_side=_psd)
                            except bt.TradeError: pass
                            _rsn = "已到止損價" if past_stop else "止損連5次掛不上、無法保護"
                            _update_trade(row_id, "failed", f"教練限價成交但{_rsn}→已市價平倉")
                            _push_owner(name, f"⚠ 教練限價平倉 · {symbol}", f"成交後{_rsn}、已即時平倉", symbol, tf=tf, event="atrade", sig="coach", d=d, sigt=str(ex.get("setup_id")))
                            continue
                        try:
                            tp_oid = client.place_close_trigger(bsym, close_side, tp_px, "tp", position_side=_psd).get("orderId")
                        except bt.TradeError:
                        ...
                        c2, ph2 = _acct._db()
                        try:
                            c2.execute(f"UPDATE trade_log SET status='open', sl_oid={ph2}, tp_oid={ph2}, msg={ph2} WHERE id={ph2}",
                                       (str(sl_oid), str(tp_oid) if tp_oid else None,
                                        "教練限價成交→已掛SL/TP" + ("（TP掛單失敗）" if not tp_oid else ""), row_id)); c2.commit()
                        finally:
                            c2.close()
                        envtag = "實盤" if client.env == "live" else "測試網"
                        _push_owner(name, f"✅ 教練限價成交 · {symbol}（{envtag}）",
                                    f"已成交、掛上止損 {_fmt_px(float(sl_s))} / 止盈 {_fmt_px(float(tp_s))}" + ("\n⚠ 止盈單掛單失敗" if not tp_oid else ""),
                                    symbol, tf=tf, event="atrade_open", sig="coach", d=d, sigt=str(ex.get("setup_id")))
                        print(f"  ✅ 教練限價成交 {client.env}: {bsym} → 掛SL/TP，轉 open")
                    elif expired:
                        if oid and oid in resting:
                            try: client.cancel_order(bsym, oid)
                            except bt.TradeError: pass
                        _update_trade(row_id, "expired", "教練限價 48h 未成交→撤單作廢")
                        _push_owner(name, f"⌛ 教練限價過期 · {symbol}", "進場區掛單 48h 未成交、已撤單作廢", symbol, tf=tf, event="atrade", sig="coach", d=d, sigt=str(ex.get("setup_id")))
                    elif oid and oid not in resting:
                        # 幽靈殘單：交易所已無此限價單、無持倉、未過期 → 清除以利重掛（防 race：>180s 才判）
                        if time.time() - (row_ts or 0) > 180:
                            _update_trade(row_id, "failed", "教練限價殘單已不在交易所→清除以利重掛")
                            print(f"  🧹 教練幽靈pending清除 {client.env}: {bsym}")
            except Exception as e:
                print(f"  ⚠ 教練限價對帳失敗 {name} {symbol}：{e}")
    except Exception as e:
        print(f"  ⚠ 教練限價對帳失敗 {symbol}：{e}")


def reconcile_coach_pending_all():
    """每監控 tick 跑：對所有 pending 教練限價列做成交→SL/TP、過期/幽靈清理。"""
    try:
        _ensure_db()
        conn, ph = _acct._db()
        try:
            rows = conn.execute("SELECT DISTINCT symbol, tf FROM trade_log WHERE source='auto' AND sig='coach' AND status='pending'").fetchall()
        finally:
            conn.close()
        for symbol, tf in rows:
            try:
                reconcile_coach_pending("crypto", "binance", symbol, tf or "15m")
            except Exception:
                pass
    except Exception as e:
        print(f"  ⚠ 教練限價對帳失敗：{e}")
...
def reconcile_manual_pending_all():
    """手動圖表限價單生命週期對帳（每監控 tick ~60s）：
    限價單未成交時幣安不接受 closePosition 止損止盈(TIF GTE 需已開倉)，故 /order 對 LIMIT 不即時掛、記 pending。
    這裡：①成交(委託單離開掛單簿 + 出現持倉) → 補掛 SL/TP、轉 open；SL 掛不上且已穿價/連5次→市價平倉(絕不裸倉)。
    ②委託單已消失且無持倉(使用者手動撤/交易所取消) → 標 cancelled。③逾 7 天未成交 → 撤單標 expired。
    只處理『有設 sl 或 tp 的 manual pending 列』；用委託單號(extra.oid)離簿+持倉雙條件判定成交，避免誤抓既有他倉。
    絕不拋例外。"""
    try:
        _ensure_db()
        conn, ph = _acct._db()
        try:
            rows = conn.execute(
                "SELECT id, acct, symbol, bsym, side, sl, tp, extra, ts FROM trade_log "
                "WHERE source='manual' AND status='pending' AND (sl IS NOT NULL OR tp IS NOT NULL)"
            ).fetchall()
        finally:
            conn.close()
        if not rows:
            return
        clients = {}                                   # 逐帳號快取 client，避免重複建
        for row_id, acct, symbol, bsym, side, sl_s, tp_s, extra_s, row_ts in rows:
            try:
                if acct not in clients:
                    c, _ = _client_for(acct)
                    clients[acct] = c
                client = clients[acct]
                if client is None:
                    continue
                ...
                try:
                    ...
                    continue
                age = time.time() - float(row_ts or 0)
                off_book = (oid is not None) and (oid not in resting)
                filled = has_pos and (oid is None or off_book)   # 出現持倉 + 委託離簿 ⇒ 成交

                def _trig(kind, vstr):
                    """補掛 closePosition 觸發單；遇殘留條件單擋路→清孤兒重試一次(hedge 不可清)。回 orderId 或 None。"""
                    px = client.quantize_price(bsym, float(vstr) * scale)
                    try:
                        return client.place_close_trigger(bsym, close_side, px, kind, position_side=_psd).get("orderId")
                    except bt.TradeError:
                        try:
                            if not _hedge:
                                client.cancel_all_algo(bsym)
                ...
                if filled:
                    sl_oid = tp_oid = None
                    if sl_s:
                        sl_oid = _trig("sl", sl_s)
                        if not sl_oid:
                            # 止損掛不上：分辨「已穿止損(本就該平)」vs「暫時性(插針已回/API抽搐)→重試別亂平」
                            mark = float((_pos or {}).get("mark") or 0)
                            stopf = float(sl_s)
                            past_stop = (mark <= stopf) if want == "long" else (mark >= stopf)
                            nfail = int(ex.get("slfail", 0)) + 1
                            if (not past_stop) and mark > 0 and nfail < 5:
                                ...
                                try:
                                    ...
                                    c3.close()
                                print(f"  ⟳ 手動止損暫掛不上(現價未到止損)、保留重試 {nfail}/5：{bsym}")
                                continue
                            # 已穿止損或連 5 次掛不上(無法保護) → 市價平倉
                            try:
                                client.close_position(bsym, position_side=_psd)
                            except bt.TradeError:
                                pass
                            _rsn = "已到止損價" if past_stop else "止損連5次掛不上、無法保護"
                            _update_trade(row_id, "failed", f"限價成交但{_rsn}→已市價平倉")
                            _push_owner(acct, f"⚠ 手動限價平倉 · {symbol}", f"成交後{_rsn}、已即時平倉", symbol, event="atrade")
                            continue
                    if tp_s:
                        tp_oid = _trig("tp", tp_s)
                    c2, ph2 = _acct._db()
                    try:
                        c2.execute(
                            f"UPDATE trade_log SET status='open', sl_oid={ph2}, tp_oid={ph2}, msg={ph2} WHERE id={ph2}",
                            (str(sl_oid) if sl_oid else None, str(tp_oid) if tp_oid else None,
                             "限價成交→已掛SL/TP" + ("（TP掛單失敗）" if (tp_s and not tp_oid) else ""), row_id))
                        c2.commit()
                    finally:
                        c2.close()
                    envtag = "實盤" if client.env == "live" else "測試網"
                    _push_owner(acct, f"✅ 手動限價成交 · {symbol}（{envtag}）",
                                f"已成交、掛上止損 {_fmt_px(sl_s) if sl_s else '—'} / 止盈 {_fmt_px(tp_s) if tp_s else '—'}"
                                + ("\n⚠ 止盈單掛單失敗" if (tp_s and not tp_oid) else ""),
                                symbol, event="atrade_open")
                    print(f"  ✅ 手動限價成交 {client.env}: {bsym} → 掛SL/TP，轉 open")
                elif off_book and not has_pos and age > 180:
                    # 委託單已不在交易所(手動撤/取消)且無持倉(防 race：建立>180s)
                    _update_trade(row_id, "cancelled", "限價委託已撤銷/消失、未成交")
                elif age > 7 * 24 * 3600:
                    # 逾 7 天未成交 → 撤殘單作廢
                    if oid and oid in resting:
                        try:
                            client.cancel_order(bsym, oid)
                        except bt.TradeError:
                            pass
                    _update_trade(row_id, "expired", "手動限價 7 天未成交→撤單作廢")
                    _push_owner(acct, f"⌛ 手動限價過期 · {symbol}", "限價單 7 天未成交、已撤單作廢", symbol, event="atrade")
            except Exception:
                continue
    except Exception as e:
        print(f"  ⚠ 手動限價對帳失敗：{e}")
...
def order(req: OrderReq):
    ...
    try:
        ...
        try:
            ...
            pass
        side = "BUY" if want == "long" else "SELL"
        _hg = _is_hedge(client)
        _psd = _posside(want, _hg)                  # hedge 帳號手動單也帶 positionSide
        ...
        def _place_trigger(kind, v):
            """掛 SL/TP 觸發單；遇殘留條件單擋路自癒：
            -4509＝該合約條件單塞滿 ~20 上限；-4130＝同方向已存在一張 closePosition 止損/止盈單。
            兩者在『此合約目前無持倉』(限價未成交) 時，殘留條件單必為孤兒 → 清掉重試一次。
            hedge 帳號不可 cancel_all_algo(會誤撤對向倉保護) → 不清、直接回報。"""
            tpx = client.quantize_price(bsym, float(v) * scale)
            try:
                client.place_close_trigger(bsym, close_side, tpx, kind, position_side=_psd)
                return None
            except bt.TradeError as e:
                if not _hg and ("-4509" in str(e) or "-4130" in str(e)):
                    try:
                        has_pos = any(p["symbol"] == bsym for p in client.positions())
                    except bt.TradeError:
                        has_pos = True
                    if not has_pos:
                        try:
                            client.cancel_all_algo(bsym)     # 清該合約孤兒條件單
                            client.place_close_trigger(bsym, close_side, tpx, kind, position_side=_psd)
                            return None
                        except bt.TradeError as e2:
                            return str(e2)
                return str(e)

        extra = None
        if otype == "MARKET":
            # 市價即時成交→已有持倉，止損止盈可立刻掛 closePosition 觸發單
            for v, kind in ((req.sl, "sl"), (req.tp, "tp")):
                if v:
                    err = _place_trigger(kind, v)
                    if err:
                        warn.append(f"{'停損' if kind == 'sl' else '止盈'}掛單失敗：{err}")
        elif req.sl or req.tp:
            # 限價未成交＝無持倉，幣安拒絕預掛 closePosition 止損止盈(TIF GTE 需已開倉, error -4509)。
            # → 記下委託單號，由背景 reconcile_manual_pending_all 偵測成交後自動補掛(每~60s)。
            try:
                _oid = (o or {}).get("orderId")
            except Exception:
                _oid = None
            extra = json.dumps({"oid": str(_oid) if _oid else None})
            warn.append("⏳ 止損止盈將於限價成交後自動補掛（約 60 秒內）")
        _log_trade(source="manual", acct=_acct._norm_name(req.name or ""), mode=client.env,
                   ...
                   extra=extra, msg="；".join(warn) or None)
