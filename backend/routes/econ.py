...
#   check_econ_events.py 的判準＝「隨時要蓋到今年年底、12 月起要蓋到明年年底」
#   （不是『往後 N 天』：BLS 前一年年底才公告下一年 → N 天門檻每年夏天都會叫一次假警報）。
# ⚠ 不要用「第 N 個週幾」外推：實測 2025-26 這 24 筆落在週二 8／週三 8／週四 6／週五 2，
#   而且 2025-10-24 差了整整兩週（當年政府停擺順延）→ 外推出來的線看起來很權威、其實是錯的。
# 2026-08-14 對過三個獨立來源（usinflationcalculator / thriveinmarkets / polymarkettrader，
# 皆註明轉載 bls.gov/schedule/news_release/cpi.htm）→ 修了 5 筆，明細見下方註記。
_CPI = {
    # ⚠ 2025-11 那一筆**不是漏了，是本來就沒有**：2025 政府停擺 → 10 月份 CPI 直接
    #    **取消、從未發布**（BLS 說無法回頭補採資料），9 月份的順延到 10/24、11 月份的順延到 12/18。
    #    → 2025 全年只有 11 次發布。舊表把那場取消的排進來，圖上等於多一條沒發生過的線。
    2025: [(1, 15), (2, 12), (3, 12), (4, 10), (5, 13), (6, 11), (7, 15), (8, 12), (9, 11), (10, 24), (12, 18)],
    # ⚠ 2/13 是**停擺順延**（原訂 2/11）—— 這種事前無法預測，正是手動表最容易錯的地方。
    #    9/11、10/14、11/10 三筆舊表是錯的（舊值 9/15、10/13、11/12），且**都還沒發生**。
    2026: [(1, 13), (2, 13), (3, 11), (4, 10), (5, 12), (6, 10), (7, 14), (8, 12), (9, 11), (10, 14), (11, 10), (12, 10)],
    # 2027：BLS **尚未公告**（官方在前一年年底才發），任何來源上的 2027 日期目前都只是「推估」→
    #        刻意留空。等 BLS 公告後再補（或設 FRED_API_KEY 讓它自己長）。
}
...
def _parse_fred_cpi(payload: dict) -> dict:
    """FRED release/dates → {year: [(month, day), ...]}。**只收 11~12 筆、每月至多一次的年份**。

    ⚠ 這條「筆數合理才採用」跟 FOMC 的「剛好 8 場才採用」是同一個保險：
    ...
    for y, lst in by_year.items():
        lst = sorted(set(lst))
        # 一年 11~12 次、且每個月最多一次。
        # ⚠ 為什麼是 11 不是 12：**一年真的可能只有 11 次** —— 2025 政府停擺，10 月份 CPI
        #   直接取消從未發布。寫死 12 會把那種年份整年判成壞資料、白白退回內建表。
        if 11 <= len(lst) <= 12 and len({m for m, _ in lst}) == len(lst):
            out[y] = lst
    return out
