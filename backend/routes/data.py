    ...
    if market == "tw":
        ...
        if timeframe in ("1m", "5m", "15m", "1h"):
            ...
            try:
                ...
                pass
            # 盤中也要還原：分割若發生在近 58 天內，盤中歷史一樣會有那個假跳空。
            try:
                from data.taiwan import tw_adjust_splits
                _df = tw_adjust_splits(_df, symbol)
            except Exception:
                pass
    ...
    try:
        if req.market == "tw" and req.symbol.upper() in FUTOPT_PRODUCTS:
            ...
            if req.timeframe in ("1m", "5m", "15m", "1h") or req.timeframe in TW_RESAMPLE:
                ...
                try:
                    from data.taiwan import tw_adjust_splits
                    df = tw_adjust_splits(df, req.symbol)
                except Exception:
                    pass
...
# （2026-09-17 SR+SMC 多空教練整個移除：smc 教練 API、教練掃描 API 與其暖掃/快取已刪。）
