...
MAX_DAYS = {
    # ★ 2026-08-15 日/週/月線 3650(10年) → 20000(約 54.8 年)：使用者要「日K 拉到最舊」。
    #   3650 讓 AAPL 只能回到 2016，但 Yahoo 其實有 1980 起的資料 —— 是我們自己截掉的。
    #   日線很輕（54 年也才約 13600 根，加密 5m 深滑常駐都 25000 根），放到資料源自己沒資料為止。
    #   ⚠ 只放寬日/週/月：intraday 那幾個是 **yfinance 自己的硬限制**（1m 僅 7 天、5m/15m/30m 59 天、
    #     1h/2h/4h 720 天），放大只會換到 yfinance 直接拒絕，不是變深。
    "1M": 20000, "1w": 20000, "1d": 20000,
