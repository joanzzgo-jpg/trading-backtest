...
#   → 每塊壓到約 1500~3000 根:跨界時受害範圍小,且暖完的自動補洞能把它補乾淨。
#   ⚠ 5m 不要壓太小:5m 深度只有 370 天,唯一會跨上線日的只有 XAUT(2026-03-26)。壓到 5 天的話
#     一次全量暖機要 74 塊/標的 × PAUSE 1.2s ≈ 6 分鐘純等待(原本 15 塊)。10 天是平衡點:
#     單塊 2880 根(內部分頁約 2 次請求=小突發,遠低於 Binance 10 次/秒),全量約 3 分鐘。
TF_CFG = {"5m": (370, 10), "4h": (4000, 120), "1d": (4500, 1000)}
PAUSE = 1.2          # 每塊間隔秒(避限流;Binance fapi 10 次/秒/IP,撞到會全域熔斷 60s 且重試 +10s)
