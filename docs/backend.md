...
### 台股報價列：基底 + 疊價（2026-09-18 定版）
使用者回報「收盤了但你們的價跟我看的有誤差」的完整解法，三件事缺一不可：

| 層 | 誰做 | 特性 |
|---|---|---|
| 基底清單 | `_tw_ticker_worker` 每 30 秒抓 TWSE/TPEX/興櫃 opendata | **整包覆蓋**；收盤後很久還是**昨天**的（實測 9/18 14:52 仍給 2330=2425＝昨收，今日官方收 2460） |
| 即時疊價 | `_tw_rt_overlay_worker` 盤中每 5 秒打 **cnyes**（備源 MIS） | 使用者正在看的＋前 50 高量＋輪掃，每輪 1000 檔 → **約 15 秒**覆蓋全清單 |
| 疊價記憶 | `live_data._TW_OV` | 疊過的值記下來，**每次基底覆蓋前重貼**（`update_tw`）；跨交易日清空 |

**2026-09-19 主源由 TWSE MIS 換成 cnyes 批次報價**（`cnyes_futures.fetch_tw_quotes_bulk`）。實測對照：

| | 一個請求吃幾檔 | 每檔傳輸(gzip) | 500 檔耗時 |
|---|---|---|---|
| MIS | 100 | 82 bytes | ~11 秒 |
| cnyes | **500** | **36 bytes** | **0.67 秒** |

- 正確性：抽樣 120 檔對證交所官方價 **106/106 完全相同**；MIS 沒有成交價（`z="-"`）的那幾檔，
  cnyes 的值與 **Yahoo 逐檔一致**。
- ★ 換源的真正理由是**正確性不是速度**：MIS 收盤後對冷門股只給買賣盤 → 我們只能估，
  實測線上有 **71 檔（3%）停在 9/18 盤中的值**、看起來就是「好幾天前的報價」（使用者回報）。
- ⚠ **不假設 cnyes 即時**：每筆報價自帶時間戳（`200007`）→ `tw_quotes_lag()` 量「整批最新那筆距現在幾秒」，
  盤中超過 180 秒就判定它落後 → 退回 MIS 十分鐘。免費報價源最常見的壞法就是**安靜地延遲 15~20 分鐘**
  （畫面上完全看不出來，數字照樣在跳）。
- ⚠ **上市/上櫃是 `TWS:`、興櫃是 `TWG:`**：只用 TWS 問，興櫃那 343 檔會整批查無、只剩落後一天的基底價。
  前綴記在 `_TW_PREFIX`（沒看過的先當 TWS、查無再用另一個前綴補問一次）→ 暖機後分組送出、不多花請求。
  實測覆蓋 **2659/2697**（興櫃 343/343，抽驗 8 檔與 Yahoo 完全相同）；剩下的 37 檔是停牌/下市，
  記進 `_TW_NOQ` 30 分鐘內不再問。
- ⚠ 「回應裡沒這檔」與「有這檔但今天還沒成交」要**分開**：後者會出現在回應裡但沒有價格，
  當成查無就會害它一開始交易也不被更新。
- ⚠ 一個請求最多 500 檔（網址長度，501 檔起 HTTP 400）。整批逾時會在下一輪自動補問（不標記成查無）。

- ★ **沒有「疊價記憶」的話，基底每 30 秒就把今日價洗回昨日價** → 非熱門股在今日/昨日之間來回跳，
  收盤後更是整批停在昨日收盤＋昨日漲跌幅。
- ★ **收盤補齊的判準是「今天補齊了沒」（進度），不是時鐘窗口**：寫成「13:35~14:30 才補」的話，
  14:52 部署重啟就補不到了（疊價記憶在記憶體、隨重啟清空）——實測那次線上抽樣 10 檔有 8 檔退回昨天。
  現在收盤後只要沒補齊就一直補（12 秒一輪、約 9 輪補完 2700 檔），補完自動停、不再打 MIS。
- ★ **估價不可以蓋掉官方值**（`live_data.tw_rt_put`）：MIS 收盤後對冷門股 `z` 是 "-"（只剩買賣盤），
  `_mis_today_price` 會用買賣區間估一個（6697 買 41.45／賣 41.50 → 估 41.475）。這種值只有在
  「基底還是昨天那份」時才比基底好 → 判準＝基底價還等於 MIS 給的昨收(`y`) 才套，否則放手。
