...
- **雷達頭頂偵測 `overhead`（2026-07-16）**：對流「在使用者正上方初生」時周圍雨量站全乾、
  移動法沒東西追，只有雷達看得到（雨還沒落地/雨量計要等累積+10分更新）。`_radar_overhead()`
  抓 **RainViewer** 最新過去幀（免金鑰，nowcast 已停供）在使用者座標 3x3 像素（z7≈1.2km/px）
  的最大 dBZ；色盤實測=Universal Blue（`_RV_UB_DBZ` 對照表由官方 CSV 生成，已用新店豪雨現場
  校準）。≥25dBZ 且腳下沒在下 → `overhead={dbz,level,age_min}` ＋ imminent 升級為
  「雷達回波已在你正上方」（`overhead:true`，前端排在 approaching ETA 之前＝最急迫）。
  幀齡 >20 分或任何失敗一律靜默略過（依賴 pillow，requirements 已加；快取：maps.json 2 分、
  圖磚依幀 path 失效）。dBZ→雨勢分級 `_dbz_level`（<32小/<40中/<47大/≥50豪，Marshall-Palmer 粗換算）。
