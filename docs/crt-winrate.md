...
> SS 是與 S1~S12 **平行的另一套訊號群**，刻意**不**混入 S 的合併時間軸：自己的合計、自己的敗後停手、自己的近期勝率。後端 `_SS_KEYS = ("ss1", "ss2")`、`SIG_KEYS` 含 `*_SS_KEYS`。前端用「系列切換」鈕（S ↔ SS）與主圖右上「標記系列」鈕（全部／只 S／只 SS）控制顯示。
...
### SS1 / SS2：依「B 棒收盤深度」拆分（同一觸發）
- SS1 與 SS2 **觸發條件完全相同**（上述 2 棒軌道反轉），只用 B 棒收盤價落在「軌道↔中軌」哪一半來區分：
  - **做多界＝`(下軌+中軌)/2`**：`B收 < 界` → **SS1**（靠下軌、較深的反轉）；`界 ≤ B收 < 中軌` → **SS2**（靠中軌、較淺）。
  - **做空界＝`(上軌+中軌)/2`**：`B收 > 界` → **SS1**（靠上軌）；`中軌 < B收 ≤ 界` → **SS2**。
  - 實作：crt.py 迴圈內 `ss_mid_lo=(b_lo_band+b_mid)/2`、`ss_mid_up=(b_up_band+b_mid)/2`，依方向選 `ss_key`。
- SS2 前端標記：圖示 `⇌`，霓虹紫 `#c724b1`（空）／霓虹綠 `#39ff14`（多），標籤 空ˢ²／多ˢ²。
- SS1+SS2 同屬 SS 系列：合計、敗後停手、近期勝率都**合在一起**算（共用 `_SS_KEYS`）。
