...
**位置＝「時間匡」（`.tf-group`）**外面**的右邊**，`.topbar-tf` 的第二個子元素
（2026-09-23 使用者：「時間倒數放到1m右邊」→「不是放到同一個匡裡」→「是整個時間匡的右邊」）。
依**目前時間框架**算（5m 就是 5 分鐘的倒數）。字級 15px/800。

- ⚠ **不要放進 `.tf-group`**：那是「凹陷軌道」樣式的框，放進去會變成軌道裡的一格、看起來像多一顆時框按鈕。
- ⚠⚠ **加進 `.topbar-tf` 會改變上方列的換行判斷**：倒數 +100px 之後，**1536 寬**（常見 1920×125% 筆電）
  的上方列從 38px 變 70px＝**被擠成兩行**（`check_topbar_rows` 抓到）。
  修法＝`@media (min-width:1500px)` 裡加 `.topbar:not(.tf-centered) .topbar-right` 可縮可捲 ——
  **置不了中就代表時框還在流排裡佔寬度**，此時右側按鈕比照窄螢幕處理。
  ⚠ 沿用 1181~1499 那段的寫法：`justify-content` 維持 `flex-start` ＋ 第一個子元素 `margin-left:auto`，
  **不可以用 `flex-end`**（溢出時前幾顆會被推到可捲區外、永遠捲不到）。
- ⚠⚠ **手機上它會被擠出視窗**：`.tf-group` 是 `flex:0 0 auto`、內容寬就佔滿整列
  → 實測 390 視窗時倒數被推到 x=394（**整塊在畫面外、不報錯**）。
  修法＝`@media (max-width:1180px)` 讓時間匡可縮（`flex:1 1 auto; min-width:0`）、倒數自己不縮。
- ⚠⚠ 判準**一定要驗「還在視窗內」**，只驗「在框右邊」會空過（`x >= 框右緣` 對畫面外的東西也成立）。
  用 `elementFromPoint` 打中心點命中自己最準。實測 1440/1200/390/360 四種寬度都通過。
...
- ⚠ **寬度固定**（使用者：「已儲存到雲端左邊要留白不被倒數影響到位置」）：`tabular-nums`（數字等寬）
  ＋ `min-width`（空字串與最長的 `23:59:59`／`29天23時` 同寬）。實測 7 種長度下 `#acctSyncState` 的 x 完全不動。
  ⚠⚠ **改字級一定要重量 min-width**：2026-09-23 把 10.5px 放大到 12.5px（使用者：「時間倒數不夠明顯」），
  `min-width` 還留著 54→62px → 「23:59:59」量到 74.8px，雲端狀態的 x 從 515.7 被推到 528.4。現在是 78px。
- **更明顯**（2026-09-23）：12.5px／700、字色 `--text`、外加一層 accent 淡底＋細框的小晶片；
  `:empty` 時底與框都透明（休市不顯示時不會留一個空框，但**位置仍佔著**）。
  最後 10 秒 `[data-soon="1"]` 整塊轉成 `--accent`（由 `_barCountdownTick` 依 `_bcSec` 設）。
  ⚠ `data-soon` 只有真的變了才寫，每秒無條件寫會讓 transition 一直重跑。
- ⚠⚠ **測這塊的顏色要先關掉 `transition`**，不要用「等一下再讀 computed style」：
  headless 的過渡不照牆上時間推進（rAF 被節流）—— 實測等 400ms 讀到 59%、等 900ms 反而只剩 6%，
...
topbar 右側一顆**有鬧鐘才出現**的鈴鐺（同 orderbook/footprint 那幾顆的做法），角標＝還沒觸發的數量；
點開是**跨標的**的清單：哪一檔、什麼價、等待中/已觸發、點一下跳過去、✕ 刪掉、一鍵清掉已觸發。

- ⚠ 為什麼要常駐入口：快捷列那顆鈴鐺（`#btnLineAlert`）只有「選取了一條水平線」時才出現 →
  設在別的標的上的鬧鐘**完全沒地方看**，也不知道總共設了幾個。
  後端 `alerts/list` **不帶 symbol 就回全部**，原本前端沒用到。
- ⚠⚠ **`.tb-icon-btn` 有 `display:flex`，會蓋過 `hidden` 屬性**（`hidden` 只是把 display 設成
  none 的*預設值*，任何 display 規則都壓得過）。沒補 `[hidden]{display:none!important}` 的話
  「沒鬧鐘就不顯示」完全沒生效 —— 按鈕一直都在、還多佔右側 30px。
  ★ 這類「條件式按鈕」一律要檢查它的 class 有沒有自己的 `display`。
- ★ **只有一支網路請求**：`_alFetchAll()`（不帶 symbol＝全部）。「目前這個標的」由
  `_alApplyCurrent()` 從那份**本機篩選**出來，換標的完全不打網路。
  ⚠ 加清單的第一版沒想到這件事：換標的打一次、60 秒那輪 `_alFetch(true)` + `_alFetchAll()`
  又各打一次 —— 實測 81 秒內 **5 次**（切 3 次標的 3 次 ＋ 靜置 2 次），其中 4 次是重複的。
  收斂後同樣情境 **1 次（−80%）**，切標的 3 次 → **0 次**。
  ⚠ `_alDel` 要**同時**更新 `_alerts`（畫在圖上的）與 `_alAll`（清單/角標）——
  只更新一份的話另一份要等 60 秒才對，而且**不會報錯**。
- ⚠ 加按鈕到 topbar 右側後，`check_topbar_rows` / `check_topbar_reachable` 都要跑。
  **而且要另外驗「按鈕顯示時」**：守門員跑的是全新 profile＝沒有鬧鐘＝按鈕隱藏 → 會**空過**。
  實測強制顯示後八種寬度仍單列、不重疊，最小空隙正好 12px＝`_TB_GAP`。

### 時框置中的判定（`ui.js` `_tbFitTf`）

- ⚠⚠ 右側佔多少**要從右緣直接量**（`bar.right − 最左那顆可見按鈕的 left`），
  **不要用「padding + 子元素寬度」推算它從哪裡開始**：2026-09-23 實測那個模型少算約 36px ——
  公式以為右側內容從 1090 開始、實際 1068，時框右緣 1065 只剩 **3px**（`_TB_GAP` 明明是 12）。
- ★ 從右緣起算仍是**位置無關**的量（右側內容永遠靠右），所以置中與否都量到同一個值 ——
  不會出現「置中→變窄→不置中→變寬→置中」的來回跳。
