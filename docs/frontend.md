...
**位置＝時框列尾端、最後一顆時框按鈕的右邊**（2026-09-23 使用者：「時間倒數放到 1m 右邊」；
原本在狀態列的訊號與雲端狀態之間）。它講的就是「目前這個時框的那根還有多久收」，跟時框放一起最好懂。
依**目前時間框架**算（5m 就是 5 分鐘的倒數）。

- ⚠ 測「有沒有排在時框後面」時，基準要用**最後一顆看得到的**時框按鈕，不可以寫死 `1m`：
  手機版的時框是可自選的（`_mobileTFs`），1m 常常沒顯示、`getBoundingClientRect` 是 `0x0`
  → 拿它當基準會**空過**（`0 >= 0` 恆成立）。實測桌機最後一顆是 1m、手機是 15m。
