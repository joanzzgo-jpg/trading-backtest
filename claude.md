...
⑤**延伸位畫反邊**（同日第二輪，使用者：「斐波那契延伸反了」）：層級原本只存一個 `lvl`，
  再用 `p1 + (p2-p1) * (1 - lvl)` 換算 —— 那個 `1 - lvl` 讓 lvl>1 落在**起點外側**
  （走勢來的方向）→ 畫一段下跌，目標價卻標在更高的地方。延伸位＝**目標價**，必須在
  **終點外側**。修＝改存 `frac`（佔整段走勢的幾成：0＝起點／1＝終點／>1＝終點外）＋
  `label`（標籤印的百分比）。★ **讓「容易搞反的換算」消失，比在換算上加註解有效** ——
  存 frac 之後，想畫在終點外 frac 就是 >1，看數字就知道對不對。
⚠⚠ ⑤ 的判準要比**方向**（`sign(px-p2) === sign(p2-p1)`）不是比大小：寫死「要比 p2 大」
  只對上漲段成立；而且**只測一個方向不夠**，正負號寫錯時其中一種走勢照樣通過 → 兩種走勢都要畫。
