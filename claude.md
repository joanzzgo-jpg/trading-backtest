...
- **`_solve` 精簡模式**：`_calc_crt_winrate(_solve=target)`（target 字串 mid/band/rr）——每訊號只掃選定目標、跳過 est/RR/全部統計，只跑敗後停手模擬回 `{win_rate, total}`。求解（達標止損）13× 掃描用此模式（偵測 mask 與完整版共用，~4-6× 快）。
...
4. `_build_target_stats` 的 `out` dict 加 `"sXX": per_sig["12"]`
...
> **強化版（variant）已移除**（2026-05，前端早已關閉、後端純屬浪費）。後端不再有 `_v` 計數/streak/cond/stop/recent 與 `out["variant"]`，`signals` 不再帶 `v`；`_solve` 改為單純 target 字串。新增訊號**不需**處理 variant。前端 `winrate.js` 仍有已中性化（強制 base）的 variant 死碼，暫留不影響功能。
...
- **主函數** `_calc_crt_winrate(df, stop_buffer_pct=0, long_only=False, _solve=None)`，**三目標各一份完整統計**：
  ...
  - 每個 target 含：`total/wins/win_rate`、`short/long`、各訊號 `abc/ab/s3~s11`、`stop_strategy`（敗後停手，內含 `.est`）、`recent100`（近 ~100 筆勝率）、`est_*`（預估盈虧比達標）、`max_loss_streak`、cond（連敗機率，在 short/long 內）。（強化版 variant 已移除）
  - `signals`：每筆 `{t, d, k, r/ot(中軌), r_b/ot_b(帶軌), r_rr/ot_rr(1:1), est_r/est_r_b, rr(預估盈虧比值,供前端顯示), stop(實際止損價,含 buffer/多棒取極值)}`。
  - 另含 `from_date`、`recent`（最近 30 筆）、`long_only`。
- **`_solve=target`**（字串 mid/band/rr）：求解專用精簡模式——只掃選定 target、跳過 est/RR/全部統計，只跑敗後停手模擬回傳 `{win_rate, total}`（偵測 mask 與完整版共用）。詳見「效能優化」。
...
- cache key：`crt_wr69:market:symbol:exchange:timeframe:buffer:long_only`（訊號邏輯／輸出結構變更時遞增版號）；solve 另用 `crt_solve5:...`；enrich 後的 df 另快取 `crt_df2:...`（不含 buffer，換 SL 緩衝免重抓，抓資料佔總時間 90%+）。以上三者皆存於 `data_cache` 池（見「效能優化」）。
...
- **左固定區 `.tb-wr-fixed-left`**（z-index:3 不隨滾動移動）
  - `#wrTargetToggle`：單鍵 toggle 切換目標（三段循環 中軌 → 上/下軌 → 1:1 → 中軌）
    ...
    - 後端三種目標各算一份完整統計：mid 放頂層、band 在 `d.band`、rr 在 `d.rr`（結構相同：總/各訊號/連敗/敗後停手）。前端用 `_wrPickView(d)`/`_wrResultKey()`/`_wrOtKey()` 三幫手依 view 取值
...
回退 Open-Meteo（全部失敗才回 503）。前端 `effects.js` 用 `navigator.geolocation`
...
- **天氣動畫類型**（前端 effects.js `dXxx`）：`sunny/night/cloudy/fog/rain/snow/storm/thunder`
...
- **`_asset_ver`（main.py）含 `effects.js` mtime**：effects.js 由 main.js 動態獨立載入（不在 bundle），
  版號須隨它變，否則 `/static` 的 immutable 長快取會吃到舊檔。
