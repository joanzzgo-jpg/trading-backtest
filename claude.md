...
#### 訊號六（S6）：CRT 放量確認單棒（CRT + 成交量 ≥ 前 20 根均量 × 1.5）
邏輯：CRT 是結構性反轉燭型（紅K→綠K 且綠K 高過紅K高，反向亦然），用 volume ≥ 前 20 根均量 × 1.5 確認「真有大量買/賣盤介入」而非低量假突破。
- **訊號棒（i）**：`crt == ±1` 同時 `volume[i] ≥ vol_prior_ma_20[i] × 1.5`
  - `vol_prior_ma_20` = 前 20 根（不含當前）成交量均值，用 `pd.Series(vol).shift(1).rolling(20).mean()`
  - 常數：`VOL_LOOKBACK = 20`、`VOL_MULT = 1.5`（crt.py）
- **方向**：跟 CRT 同向（-1 做空 / +1 做多）
- 進場：訊號棒下一根（`entry_i = i + 1`）
- 停損：**訊號棒**最高（做空）/ 最低（做多）
- 前端圖示：◇（紫藍 `#9fa8da`，極簡模式 `#3949AB`）；標籤：空⁶／多⁶
- **實測勝率**（跨標的/時框驗證，避免過擬合）：
  - BTC 1d (2021-2024)：75.7%（20 短 + 17 多）
  - SOL 1d (2022-2024)：78.6%（15 短 + 13 多）
  - BTC 4h (2023-2024)：78.8%（131 短 + 110 多，統計顯著）
  - BTC 1h (2024)：69.3%（202 短 + 231 多）
  - ETH 1d (2021-2024)：61.8%（21 短 + 13 多，弱勢但仍 > 50%）
